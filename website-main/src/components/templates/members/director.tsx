...
const employment = [
  ...
  {position: 'Lecturer', positionKo: '강사', department: 'Department of Electronic and Semiconductor Engineering', departmentKo: '전자반도체공학부', organization: 'Kangnam University', organizationKo: '강남대학교', period: '2025.03 – 2026.02', logo: logoKangnam, isCurrent: true},
...
export const MembersDirectorTemplate = () => {
  ...
  // Count total periods (semesters) instead of courses
  const lecturerPeriodCount = useMemo(() => 
    lecturerCourses.reduce((sum, course) => sum + course.periods.length, 0), [lecturerCourses])
  
  const taPeriodCount = useMemo(() => 
    taCourses.reduce((sum, course) => sum + course.periods.length, 0), [taCourses])
  
  const totalPeriodCount = useMemo(() => 
    lecturerPeriodCount + taPeriodCount, [lecturerPeriodCount, taPeriodCount])
              ...
              <div className="relative pl-24 md:pl-32 border-l-2 border-primary/20">
                {education.map((edu, index) => (
                  <div key={index} className="relative pb-32 last:pb-0 group">
                    {/* Timeline dot - positioned centered on card */}
                    <div className="absolute -left-[30px] md:-left-40 top-1/2 -translate-y-1/2 size-12 md:size-16 bg-primary rounded-full border-3 md:border-4 border-white shadow-md transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-primary/30"/>
                      ...
                      {expandedProjectYears.includes(year) && (
                        <div className="border-t border-gray-100 divide-y divide-gray-50">
                          {projectsByYear[year].map((project, index) => {
                            ...
                            // Determine role
                            const role = project.roles.principalInvestigator 
                              ? 'PI' 
                                ...
                                <div className="flex items-start gap-12 md:gap-16">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-6 mb-8">
                                      ...
                                      <span className={`px-6 py-2 text-[9px] md:text-[10px] font-bold rounded-full ${roleColors[role]}`}>
                                        {role}
                                      </span>
                                    </div>
                                    <p className="text-xs md:text-sm font-bold text-gray-900 line-clamp-2">{project.titleKo}</p>
                                    <p className="text-[10px] md:text-xs text-gray-600 mt-2 line-clamp-2">{project.titleEn}</p>
                                    <p className="text-[10px] md:text-xs text-gray-500 mt-4">{project.fundingAgencyKo} ({project.fundingAgency})</p>
                                  </div>
                                </div>
                              </div>
                            )
            ...
            {lectures.length > 0 && (
              <section>
                <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-16 md:mb-24 flex items-center gap-8">
                  <span className="w-1 h-20 bg-primary rounded-full" />
                  Teaching
                  <span className="px-8 py-2 bg-gray-200 text-gray-600 text-[10px] font-bold rounded-full ml-4">{totalPeriodCount}</span>
                </h3>
                ...
                {lecturerCourses.length > 0 && (
                  <div className="mb-24">
                    <div className="flex items-center gap-8 mb-12">
                      ...
                      <span className="px-8 py-2 bg-amber-400 text-gray-900 text-[10px] font-bold rounded-full">{lecturerPeriodCount}</span>
                    </div>
                    <div className="space-y-12">
                      {lecturerCourses.map((course, index) => (
                        <div key={index} className="bg-white border border-gray-100 rounded-xl p-16 md:p-20 hover:shadow-md hover:border-primary/30 transition-all">
                          <div className="flex items-start gap-12 md:gap-16">
                            <div className="size-36 md:size-40 rounded-xl flex items-center justify-center shrink-0 bg-amber-400 text-gray-900">
                              <BookOpen size={18} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-6 mb-8">
                                {course.periods.map((period, i) => (
                                  <span key={i} className="px-8 py-2 bg-amber-100 text-amber-700 text-[9px] md:text-[10px] font-bold rounded-full">
                                    {period}
                                  </span>
                                ))}
                              </div>
                ...
                {taCourses.length > 0 && (
                  <div>
                    <div className="flex items-center gap-8 mb-12">
                      ...
                      <span className="px-8 py-2 text-white text-[10px] font-bold rounded-full" style={{backgroundColor: '#e8879b'}}>{taPeriodCount}</span>
                    </div>
                    <div className="space-y-12">
